...
export type StudentUpsertPlan =
  | { action: "create"; fields: { ci: string; nombres: string; apellidos: string; passwordHash: null } }
  | { action: "update"; fields: { nombres: string; apellidos: string } };
 ...
 * and can't clobber a student who already activated their account.
 */
export function planRosterRow(
  ...
  if (existing) {
    return { action: "update", fields: { nombres: row.nombres, apellidos: row.apellidos } };
  }
  return {
    action: "create",
    fields: { ci: row.ci, nombres: row.nombres, apellidos: row.apellidos, passwordHash: null },
  };
}
